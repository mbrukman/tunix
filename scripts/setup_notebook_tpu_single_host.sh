...
#
# Example usage:
#   bash single_host.sh --tpu-zone us-central1-a --tpu-project cloud-tpu-inference-test
#   or bash single_host.sh --tpu-name ${USER}-v5e-8 --tpu-zone us-central1-a --tpu-project cloud-tpu-inference-test --tpu-name ${USER}-tpu-v5e-8
...
tpu_exec() {
  local PORT_FORWARDING="${2:--L 8889:localhost:8889}"
  gcloud alpha compute tpus tpu-vm ssh --zone="$TPU_ZONE" --project="$TPU_PROJECT" --verbosity=debug \
    "$TPU_NAME" --command="$1" -- -o ProxyCommand='corp-ssh-helper -relay=mtv5.r.ext.google.com %h %p' $PORT_FORWARDING
}
...
tmux new -d -s jupyter-session "$(declare -f tpu_exec); tpu_exec 'bash ~/run_jupyter.sh' > ~/jupyter.log 2>&1"
}
