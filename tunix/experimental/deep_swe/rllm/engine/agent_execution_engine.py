...
class AgentExecutionEngine:
    ...
    async def get_model_response(self, prompt, application_id, **kwargs):
        ...
        prompt_str =  self.chat_parser.parse(prompt, add_generation_prompt=True, is_first_msg=True)
    ...
    def update_envs_and_agents(self, envs, agents):
        ...
        print(f"Total tasks to run: {self.n_parallel_agents}")
    ...
    async def run_agent_trajectory_async(self, idx, application_id, seed=0, mode="Text", **kwargs):
        ...
        assert mode == "Text", "Currently only Text mode is supported for agent execution engine but received: {}".format(mode)
    ...
    async def trajectory_generator(self, reset_seed=0, timing_raw=None, mode="Text", **kwargs):
        ...
        async def launch_one_trajectory_task(env_idx: int):
            try:
                ...
                raise e
            return result
        print(f"Running {len(self.envs)} trajectories with max concurrency of {max_concurrency}...")
