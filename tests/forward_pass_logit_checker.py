...
"""Check if a Tunix supported model's logits match the golden logits from a Hugging Face model."""
# Example cmd: python -m tests.forward_pass_logit_checker --model_name=gemma2-7b-it --max_kl_div=0.015
...
def get_data(golden_data, golden_data_index, config):
    ...
    print(
        f"config.global_batch_size_to_train_on={config.global_batch_size_to_train_on}"
    )
    ...
    print(
        f"ids={ids}, decoder_segment_ids = {decoder_segment_ids}, decoder_positions= {decoder_positions}"
    )
...
def main(test_args):
    ...
    print(
        f"Tokenizer vocab size: {tokenizer.vocab_size} vs {tunix_tokenizer.vocab_size()}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_target_length", type=int, required=False, default=1024)
